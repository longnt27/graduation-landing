...
export function validateRsvp(body = {}) {
  ...
  if (arrivalTime) {
    ...
    if (totalMinutes < minMinutes || totalMinutes > maxMinutes || minute % 15 !== 0) {
      throw new Error('Giờ dự kiến phải từ 07:00 đến 12:00 và theo bước 15 phút.');
    }
  }
...
export function validateGuestbook(body = {}) {
  ...
  const isPublic = body.isPublic !== false && body.isPublic !== 'false' && body.isPublic !== 0 && body.isPublic !== '0';
  ...
  return { spam: false, data: { name, title, message, image: image || null, isPublic } };
}
